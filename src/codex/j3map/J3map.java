...
public class J3map implements Cloneable {
    ...
    /**
     * Reads the asserted <code>InputStream</code>. This is the main method used
     * for reading a j3map file.
     *
     * @param stream
    ...
    /**
     * Returns the Object stored at the key. If the object does not exist or is
    ...
    /**
     * Get the property stored at the key. The property, if an instance of
     * <code>classtype</code>, is cast to <code>classtype</code> then returned.
     * Otherwise, returns null.
     * <p>
     * Can be used to get properties processed by custom processors, but does
     * not use <code>J3mapPropertyProcessor.get(...)</code> to cast the
     * property. For more reliability and precision, use
     * <code>getProcessor(...).get(...)</code>.
     *
     * @param <T>
     * @param classtype
     * @param key
...
//		else return (T)processor.get(this, key);
     ...
     * @param key
     * @param defaultvalue
     * @return if property found, returns the found property, otherwise returns
     * the default value.
     */
...
//		else return (T)processor.get(this, key);
    ...
    public J3map getJ3map(String key) {
        ...
        if (obj != null && obj instanceof J3map) {
            return (J3map) obj;
        }
        else {
            return null;
    ...
    /**
     * Get the array stored at the key. Use this method (or similar methods) to
     * access arrays since arrays wrapped in <code>ArrayWrappers</code>.
     *
     * @param <T>
     * @param classtype
     * @param key
    ...
    /**
     * Get the Float stored at the key. If the object at the key is null or not
     * a float, returns null.
     *
     * @param key
     * @return
     */
    ...
    /**
     * Returns the float stored at the key. If the object does not exist or is
     * null or is not a float, returns the default value.
     *
     * @param key
     * @param defaultvalue
     * @return
    ...
    /**
     * Get the Integer stored at the key. If the object at the key is null or
     * not a integer, returns null.
     *
     * @param key
     * @return
     */
    ...
    /**
     * Returns the integer stored at the key. If the object does not exist or is
     * null or is not an integer, returns the default value.
     *
     * @param key
     * @param defaultvalue
     * @return
    ...
    /**
     * Get the String stored at the key. If the object at the key is null or not
    ...
    /**
     * Returns the String stored at the key. If the object does not exist or is
     * null or is not a String, returns the default value.
     *
     * @param key
     * @param defaultvalue
     * @return
    ...
    /**
     * Get the Boolean stored at the key. If the object at the key is null or
     * not a boolean, returns null.
     *
     * @param key
     * @return
     */
    ...
    /**
     * Returns the boolean stored at the key. If the object does not exist or is
     * null or is not a boolean, returns the default value.
     *
     * @param key
     * @param defaultvalue
     * @return
    ...
    /**
     * Get the String[] stored at the key. If the object at the key is null or
     * not a String[], returns null.
     *
     * @param key
     * @return
     */
    ...
    /**
     * Returns the String[] stored at the key. If the object does not exist or
     * is null or is not a String[], returns the default value.
     *
     * @param key
     * @param defaultvalue
     * @return
    ...
    // storing
    /**
     * Stores the asserted property at the key if no property is currently
     * stored at the key.
     *
     * @param key
     * @param property
    ...
    /**
     * Replaces the old key with a new key. Involves deleting the old property
     * and immediately restore it at the new key.
     *
     * @param oldkey
     * @param newkey
     */
    ...
    /**
     * Checks if all properties represented by the property array are
     * contained in this J3map.
     * @param properties
     * @return true if all exist
     */
    public boolean propertiesExist(Property... properties) {
        for (Property prop : properties) {
            Property p = this.properties.get(prop.key);
            if (p == null || !p.property.equals(prop.property)) {
                return false;
            }
        }
        return false;
    ...
    /**
     * Requires that all asserted property mappings exist. A more severe version
     * of <code>propertiesExist(...)</code> in which an exception is thrown if a
    ...
    /**
     * Checks if the property at the asserted key is assignable to the asserted
     * class type.
     *
     * @param type
     * @param key
    ...
    /**
     * Executes the command. If the property at the key exists, passes the
     * property into the command. Otherwise, passes the default value into the
     * command.
     *
     * @param <T>
     * @param key
    ...
    /**
     * Sets the name of this j3map. If this j3map is stored by another j3map,
     * then this will also change where this j3map is stored in the parent j3map
     * to the new name.
     *
     * @param name
     */
    ...
    // getters
    /**
     * Get the path of the file this j3map was loaded from. Does not work if
     * this j3map was loaded from an InputStream or created by another j3map as
     * an inner j3map. Use <code>setSourcePath(...)</code> to artificially set
     * the source path if the path is null.
     *
     * @return
     */
    ...
    public HashMap<String, Property> getProperties() {
        HashMap<String, Property> clone = new HashMap<>();
    ...
    /**
     * Get the parent <code>J3map</code> of this map. Returns null if this map
     * has no parent.
     *
     * @return
     */
    ...
    // iterating
    /**
     * Execute the BiConsumer for each property stored in this j3map. Is not
     * parse order sensitive.
     *
     * @param foreach
     */
    public void forEach(BiConsumer<String, Property> foreach) {
        properties.forEach(foreach);
    }

    /**
     * Execute the BiConsumer for each property of the asserted class type in
     * this j3map. Is not parse order sensitive.
     *
     * @param <T>
     * @param classtype
     * @param foreach
    ...
    /**
     * Execute the Consumer for each property stored in this j3map. Is parse
     * order sensitive.
     *
     * @param foreach
     */
    ...
    /**
     * Execute the Consumer for each property of the asserted class type in this
     * j3map. Is parse order sensitive.
     *
     * @param <T>
     * @param classtype
     * @param foreach
    ...
    // printing
    /**
     * Prints out the list of properties and keys. Includes properties and keys
     * from all inner maps.
     */
    ...
    /**
     * Prints out the list of properties and keys. Only prints properties that
     * this map stores directly.
     */
    ...
    /**
     * Prints out a clean list of properties and keys. Includes properties and
     * keys from all inner maps.
     */
    ...
    /**
     * Prints out a clean list of properties and keys. Only prints properties
     * that this map stores directly.
     */
    ...
    // Object overrides
    /**
     * Creates a clone of this j3map. The properties themselves are not cloned.
     *
     * @return
     */
    @Override
